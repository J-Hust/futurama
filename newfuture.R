...
mychars <- c("Fry", "Bender", "Leela", "Farnsworth", "Amy", "Hermes", "Zapp", "Kif", "Calculon", "Cubert", "Lrrr", "Nixon", "Mom", "Robot Devil", "Elzar", "Coilette", "Morbo", "Linda", "Clamps", "Donbot", "Joey", "Flexo", "Scruffy")
...
for (i in 1:length(mychars)){
  boop <- paste0(mychars[i], '_done')
  assign(boop, get(paste0(mychars[i], "_lines")) 
         %>% VectorSource() 
         %>% Corpus() 
         %>% tm_map(FUN=tolower)
  ...
  assign(boop, data.frame(Word=names(Frequency), Frequency=Frequency))
}